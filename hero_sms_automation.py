...
def _claudeotp_resolve_country_id(service_id, pref):
    """Resolve the ClaudeOTP order 'country' id. `pref` may be an exact order
    country-id (e.g. 146576), a country name ('Philippines'), or ISO ('PH').
    Picks the cheapest in-stock, orderable option. This is required because the
    order endpoint needs the per-operator id from /services/<id>/countries, NOT
    the generic country id."""
    import urllib.parse
    try:
        res = _claudeotp_call("GET", "/services/" + urllib.parse.quote(str(service_id)) + "/countries")
        countries = ((res.get("data") or {}).get("countries")) or []
    except Exception:
        countries = []
    if not countries:
        return str(pref)
    p = str(pref).strip()
    pl = p.lower()
    # 1) exact order-id match -> use as-is
    for c in countries:
        if str(c.get("id")) == p:
            return p
    # 2) name / ISO match -> cheapest in-stock & orderable
    matches = [c for c in countries
               if c.get("can_order") and (c.get("stock") or 0) > 0 and pl
               and (pl in str(c.get("name", "")).lower() or pl == str(c.get("iso", "")).lower())]
    if matches:
        best = min(matches, key=lambda c: c.get("price") or 1e12)
        print(f"🌐 [API] Picked {best.get('name')} · {best.get('operator')} · "
              f"stock {best.get('stock')} · {best.get('price_formatted')} (order id {best.get('id')})")
        return str(best.get("id"))
    print(f"🔴 [API] No in-stock option matched country '{pref}' for service {service_id}.")
    return str(pref)
...
def _claudeotp_get_number():
    ...
    import urllib.parse
    try:
        country_id = _claudeotp_resolve_country_id(CONFIG.api_service_code, CONFIG.api_country_code)
        path = ("/orders?country=" + urllib.parse.quote(str(country_id))
                + "&service_id=" + urllib.parse.quote(str(CONFIG.api_service_code)))
        res = _claudeotp_call("POST", path)
        print(f"🌐 [API] POST /orders -> {str(res)[:300]}")
        if res.get("success"):
            d = res.get("data") or {}
            # The real response nests the order under data.results[0].order
            order = None
            results = d.get("results")
            if isinstance(results, list) and results and isinstance(results[0], dict):
                order = results[0].get("order") or results[0]
            if not isinstance(order, dict):
                order = d.get("order") or d
            order_id = _dig(order, "id", "order_id", "orderId")
            num = order.get("number") if isinstance(order, dict) else None
            if isinstance(num, dict):
                phone = _dig(num, "value", "formatted")
            else:
                phone = num or _dig(order, "phone", "phone_number", "msisdn", "phonenumber")
            if order_id and phone:
                print(f"✅ [API] Got number {phone} (order {order_id})")
...
def main() -> None:
    ...
    try:
        with sync_playwright() as p:
            ...
            hero = find_or_open_page(context, "about:blank" if CONFIG.use_api else CONFIG.hero_url)
            hero.bring_to_front()
            
            if CONFIG.use_api:
                print("\n🌐 API mode ON — skipping hero-sms.com login/navigation (number + OTP come from the provider API).")
            else:
            ...
            while (attempt < max_attempts or is_looping) and not success:
                ...
                try:
                    ...
                    # 2. Get a number. Website mode buys it on hero-sms.com;
                    #    API mode skips the website entirely (number from provider API).
                    top_before = None
                    if not CONFIG.use_api:
                        ...
                        print("\n🌐 API mode — getting the number from the provider API (skipping hero-sms.com)...")
