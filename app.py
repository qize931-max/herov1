...
def api_tracking():
    ...
    # Custom date / date-range from the calendar picker
    cf = (request.args.get('from') or '').strip()
    ct = (request.args.get('to') or '').strip()
    custom = None
    if cf and ct:
        try:
            s = datetime.strptime(cf, '%Y-%m-%d').replace(hour=0, minute=0, second=0, microsecond=0)
            e = datetime.strptime(ct, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
            if e < s:
                s, e = datetime.strptime(ct, '%Y-%m-%d'), datetime.strptime(cf, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
            custom = (s, e)
        except Exception:
            custom = None
    ...
    if custom:
        start, end = custom
        rng = 'custom'
        unit = 'hour' if start.date() == end.date() else 'day'
    elif rng == 'today':
    ...
    if unit == 'hour':
        ...
        if ndays > 45:
            # long ranges -> weekly buckets so the chart stays readable
            cur = start.replace(hour=0, minute=0, second=0, microsecond=0)
            while cur <= end:
                nxt = cur + timedelta(days=7)
                cnt = sum(d for (dt, d, _) in events if cur <= dt < nxt)
                buckets.append({"label": f"{cur.month}/{cur.day}", "count": cnt})
                cur = nxt
        else:
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .trk-date { background:rgba(0,0,0,0.25); border:1px solid var(--border-color); border-radius:8px; color:var(--text-main); font-family:var(--font-main); font-size:0.8rem; padding:6px 10px; outline:none; color-scheme:dark; }
        .trk-date:focus { border-color:var(--primary-glow); }
...
<body>
        ...
        <div class="modal-box" style="max-width:920px;">
            ...
            <div class="modal-body">
                ...
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:1rem;">
                    <span style="font-size:0.68rem;color:var(--text-muted);letter-spacing:.06em;text-transform:uppercase;">Pick dates</span>
                    <input type="date" id="trkFrom" class="trk-date">
                    <span style="color:var(--text-muted);">→</span>
                    <input type="date" id="trkTo" class="trk-date">
                    <button onclick="applyTrkCustom()" class="btn-primary" style="padding:6px 13px;font-size:0.78rem;">Apply</button>
                    <span id="trkOneDay" style="font-size:0.7rem;color:var(--text-muted);">tip: same date twice = single day</span>
                </div>
                <!-- KPIs (exact) -->
                <div class="trk-kpis">
    ...
    <script>
        ...
        function applyTrkCustom() {
            const f = document.getElementById('trkFrom').value, t = document.getElementById('trkTo').value;
            if (!f || !t) { document.getElementById('trkOneDay').textContent = 'pick both dates'; return; }
            loadTracking('custom', f, t);
        }
        function loadTracking(r, cf, ct) {
            trkRange = r;
            const custom = !!(cf && ct);
            [...document.getElementById('trkSeg').children].forEach(b=>b.classList.toggle('on', !custom && b.dataset.r===r));
            let url, label;
            if (custom) {
                url = '/api/tracking?from=' + encodeURIComponent(cf) + '&to=' + encodeURIComponent(ct);
                label = (cf === ct) ? cf : (cf + ' → ' + ct);
            } else {
                url = '/api/tracking?range=' + encodeURIComponent(r);
                label = TRK_LABELS[r] || r;
            }
